...
export default function Contact() {
  ...
  const inputCls = "w-full px-3.5 py-2.5 bg-white/[0.04] border border-white/[0.08] rounded-xl text-sm text-white placeholder-slate-600 focus:outline-none focus:border-blue-500/60 focus:bg-white/[0.06] transition-all";

  return (
    <section id="contact" className="py-24 relative">
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/[0.07] to-transparent" />
      <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 60% 50% at 50% 0%, rgba(59,130,246,0.06) 0%, transparent 60%)" }} />
      ...
      <div className="site-container relative">
        ...
        <div className="grid lg:grid-cols-5 gap-10">
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            <div>
              <h3 className="text-white font-semibold mb-1">Let&apos;s build something together</h3>
              <p className="text-slate-400 text-sm leading-relaxed">Currently accepting new clients for Q3 2026. Response within 24 hours.</p>
            </div>
              ...
              {channels.map(ch => (
                <a key={ch.label} href={ch.href} target={ch.href.startsWith("http") ? "_blank" : undefined}
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-slate-300 group-hover:text-white transition-colors font-medium truncate">{ch.value}</p>
                  </div>
                </a>
              ))}
            </div>
            ...
            <div className="card-surface rounded-xl p-4">
              ...
              {[
                ...
                <div key={k} className="flex justify-between py-1.5 border-b border-white/[0.04] last:border-0 text-xs">
              ...
              <form onSubmit={submit} className="card-surface rounded-2xl p-6 flex flex-col gap-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" name="name" required value={form.name} onChange={change} placeholder="Your name" className={inputCls} />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1.5">Email *</label>
                    <input type="email" name="email" required value={form.email} onChange={change} placeholder="you@company.com" className={inputCls} />
                  </div>
                </div>
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1.5">Company</label>
                    <input type="text" name="company" value={form.company} onChange={change} placeholder="Your company" className={inputCls} />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1.5">Budget Range</label>
                    <select name="budget" value={form.budget} onChange={change} className={inputCls}>
                      <option value="" className="bg-zinc-900">Select budget</option>
                      <option value="<5k" className="bg-zinc-900">&lt; $5,000</option>
                      <option value="5k-15k" className="bg-zinc-900">$5,000 – $15,000</option>
                      <option value="15k-50k" className="bg-zinc-900">$15,000 – $50,000</option>
                      <option value="50k+" className="bg-zinc-900">$50,000+</option>
                      <option value="monthly" className="bg-zinc-900">Monthly retainer</option>
                    </select>
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1.5">Message *</label>
                  <textarea name="message" required value={form.message} onChange={change} rows={5} placeholder="Tell me about your project, your team, and what kind of help you need..." className={`${inputCls} resize-none`} />
                </div>
                <button type="submit" disabled={submitting}
                  className="w-full py-3 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-semibold rounded-xl transition-all text-sm flex items-center justify-center gap-2">
                  {submitting ? (
                ...
                <p className="text-center text-xs text-slate-600">No spam. Just a genuine conversation about your project.</p>
              </form>
            )}
          </div>
        </div>
